...
def synth_one_sample(targets, predictions, vocoder, model_config, preprocess_config,
    ...
    mel_target = targets[6][idx, :mel_len].detach().transpose(0, 1)
    mel_prediction = predictions[1][idx, :mel_len].detach().transpose(0, 1)
    duration = targets[11][idx, :src_len].detach().cpu().numpy()
    if preprocess_config["preprocessing"]["pitch"]["feature"] == "phoneme_level":
        pitch = targets[9][idx, :src_len].detach().cpu().numpy()
        pitch = expand(pitch, duration)
    else:
        pitch = targets[9][idx, :mel_len].detach().cpu().numpy()
    if preprocess_config["preprocessing"]["energy"]["feature"] == "phoneme_level":
        energy = targets[10][idx, :src_len].detach().cpu().numpy()
        energy = expand(energy, duration)
    else:
        energy = targets[10][idx, :mel_len].detach().cpu().numpy()

...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name in ["LJSpeech", "Ego4D_final_v4", "LibriTTS"]:
            ...
            ### np.array, shape: (192,)
            speaker_embedding = torch.load(speaker_embedding_path).squeeze().cpu().numpy()
        if self.dataset_name in ["LJSpeech", "LibriTTS"]:
            speaker_embedding = np.zeros(192,)
        else:
            ...
            video_embedding = np.load(video_path)
    ...
    def reprocess(self, data, idxs):
        ...
        durations = [data[idx]["duration"] for idx in idxs]
        speaker_embeddings = [data[idx]["speaker_embedding"] for idx in idxs]
        video_embeddings = [data[idx]["video_embedding"] for idx in idxs]

        text_lens = np.array([text.shape[0] for text in texts])
        mel_lens = np.array([mel.shape[0] for mel in mels])
        vid_lens = np.array([vid.shape[0] for vid in video_embeddings])
        ...
        ### new
        speaker_embeddings = pad_1D(speaker_embeddings)
        video_embeddings = pad_2D(video_embeddings)
        # print('vid_lens: ', vid_lens)
        # print('video_embeddings.dtype:', video_embeddings.dtype)
